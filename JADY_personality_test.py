...
if submitted:
    ...
    if len(answers) < 30 or not user_name.strip():
        ...
        # ================= 5. 将结果发射到 Supabase 云端数据库 =================
        # 把30道选择题的答案打包成一个整体字典
        db_answers = {f"第{k}题": v for k, v in answers.items()}

        # 组装一条完美的数据库记录
        db_record = {
            "name": user_name,
            "gender": user_gender if user_gender else "保密",
            "age": user_age,
            "position": user_position,
            "main_trait": max_trait,
            "score_zq": int(score_ZQ),
            "score_tz": int(score_TZ),
            "score_ws": int(score_WS),
            "score_st": int(score_ST),
            "duration": int(duration_seconds),
            "weaknesses": f"{weak1}, {weak2}, {weak3}",
            "strengths": f"{str1}, {str2}, {str3}",
            "all_answers": db_answers
        }

        # 执行向云端数据库插数据的命令
        try:
            supabase.table("jady_results").insert(db_record).execute()
            st.success("☁️ 数据已成功同步至永久云端数据库！")
