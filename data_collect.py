...
# 🌟 门禁系统：超级管理员密码锁
admin_password = st.sidebar.text_input("🔒 请输入管理员密码解锁后台：", type="password")
if admin_password != "jady2026":  # 你可以随意修改这里的密码
    st.title("🔒 JADY 性格测试 - 智能数据看板")
    st.warning("⚠️ 这是出卷人专属的私密后台。请输入正确的管理员密码以解锁数据大屏。")
    st.stop()  # 密码不对，立刻终止程序，不加载任何数据
...
try:
    ...
    st.error(f"无法连接到云端数据库，请检查网络或密钥配置。详细报错: {e}")
    st.stop()
...
with st.sidebar:
    ...
    if st.button("💥 清空云端所有数据", type="primary", disabled=not confirm_delete):
        # 向 Supabase 发送批量删除指令 (删除 ID > -1 的所有记录)
...
if len(df_clean) > 0:
    ...
    cols.remove('记录ID')
    cols.insert(0, cols.pop(cols.index('提交时间')))
    cols.insert(1, cols.pop(cols.index('姓名')))
    cols.insert(2, cols.pop(cols.index('答题耗时(秒)')))
